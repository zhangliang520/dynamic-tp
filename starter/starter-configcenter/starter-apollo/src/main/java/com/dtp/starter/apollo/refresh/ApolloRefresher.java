...
public class ApolloRefresher extends AbstractRefresher implements ConfigFileChangeListener, InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        if (apollo != null && StringUtils.isNotBlank(apollo.getNamespace())) {
            dtpNameSpace = apollo.getNamespace();
        } else {
            String[] apolloNamespaces = this.namespace.split(",");
            dtpNameSpace = apolloNamespaces[0];
        }

        configFileType = deduceFileType(dtpNameSpace);
        namespace = dtpNameSpace.replaceAll("." + configFileType.getValue(), "");
        ...
        try {
            ...
            log.info("DynamicTp refresher, add listener success, namespace: {}", dtpNameSpace);
        } catch (Exception e) {
            log.error("DynamicTp refresher, add listener error, namespace: {}", dtpNameSpace, e);
        }
    }
